...
public class ForStatementTest {
	...
	public void testApplyToDefaultStepSomePathsContainReturn() {
		...
		State currentProcessState = ProgramFactory.createState("state1", Arrays.asList(forStatement), null);
	...
	@Test
	public void testApplyToDefaultStepSomePathsContainExit() {
		/*i, x, a, b: INT;
		 * loop precondition^ [{inv1(s0), s0, NORMAL}, {inv2(s0), s0, RETURN}]
		 * statement: FOR i := a TO b DO IF cond THEN EXIT; END_IF END_FOR
		 * expected result:
		 * [{inv2(s0), s0, RETURN},
		 * {loopinv(s0) /\ 1 > 0 /\ getVarInt(s0, i) <= getVarInt(s0, b) /\ getVarBool(s0, cond), s0, NORMAL},
		 * {loopinv(s0) /\ 1 < 0 /\ getVarInt(s0, i) >= getVarInt(s0, b) /\ getVarBool(s0, cond), s0, NORMAL},
		 *  {loopinv(s0), s0, NORMAL}]
		 * vcs = [
		 * inv1(s0) --> loopinv(setVarInt(s0, i, getVarInt(s0, a))),
