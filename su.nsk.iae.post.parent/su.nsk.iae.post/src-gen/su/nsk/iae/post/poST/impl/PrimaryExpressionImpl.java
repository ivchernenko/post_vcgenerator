  ...
  private Term generateProcessStatus(ProcessStatusExpression procStatus, Term currentState, VCGeneratorState globVars) {
		...
		if (procStatus.isStop())
			return new ComplexTerm(
					DataType.BOOL,
					FunctionSymbol.EQ,
					new ComplexTerm(FunctionSymbol.getPstate, currentState, procCode),
					su.nsk.iae.post.vcgenerator.Constant.stop);
		else if (procStatus.isError())
				return new ComplexTerm(
						DataType.BOOL,
						FunctionSymbol.EQ,
						new ComplexTerm(FunctionSymbol.getPstate, currentState, procCode),
						su.nsk.iae.post.vcgenerator.Constant.error);
			else if (procStatus.isInactive())
				return new ComplexTerm(
						DataType.BOOL,
						FunctionSymbol.OR, 
						new ComplexTerm(
