...
abbreviation c :: variable where "c \<equiv> (Suc (Suc (Suc 0)))"
abbreviation d :: variable where "d \<equiv> (Suc (Suc (Suc (Suc 0))))"
abbreviation e :: variable where "e \<equiv> (Suc (Suc (Suc (Suc (Suc 0)))))"
abbreviation q :: variable where "q \<equiv> (Suc (Suc (Suc (Suc (Suc (Suc 0))))))"

abbreviation Init :: process where "Init \<equiv> 1"
 
 abbreviation stop:: pstate where "stop \<equiv> 0"
 abbreviation error:: pstate where "error \<equiv> 1"

abbreviation Init'init :: pstate where "Init'init \<equiv> 2"
abbreviation Init'state2 :: pstate where "Init'state2 \<equiv> 3"
...
fun getVarBool:: "state => variable => bool" where
  ...
  "getVarBool (setVarAny s'  a'value) a = a'value" |
  "getVarBool (setVarAny s'  a'value) x' = getVarBool s' x'" |
...
fun getVarInt:: "state => variable => int" where
  ...
  "getVarInt (setVarAny s'  a'value) x' = getVarInt s' x'" |
...
fun getVarReal:: "state => variable => real" where
  ...
  "getVarReal (setVarAny s'  a'value) x' = getVarReal s' x'" |
...
fun getVarArrayBool:: "state => variable => int => bool" where
  ...
  "getVarArrayBool (setVarAny s'  a'value) x' i' = getVarArrayBool s' x' i'" |
...
fun getVarArrayInt:: "state => variable => int => int" where
  ...
  "getVarArrayInt (setVarAny s'  a'value) x' i' = getVarArrayInt s' x' i'" |
...
fun getVarArrayReal:: "state => variable => int => real" where
  ...
  "getVarArrayReal (setVarAny s'  a'value) x' i' = getVarArrayReal s' x' i'" |
...
fun getPstate:: "state => process => pstate" where
  ...
  "getPstate (setVarAny s'  a'value) p' = getPstate s' p'" |
...
fun substate:: "state => state => bool" where
  ...
  "substate s' (setVarAny s1'  a'value) =
    (if s' = (setVarAny s1'  a'value) then True else substate s' s1')" |
...
fun toEnvNum:: "state => state => nat" where 
  ...
  "toEnvNum s' (setVarAny s1'  a'value) =
    (if s' = (setVarAny s1'  a'value) then 0 else toEnvNum s' s1')" |
...
fun ltime:: "state => process => nat" where 
  ...
  "ltime (setVarAny s'  a'value) p' = ltime s' p'" |
...
definition VC2 where
  "VC2 inv' env' Init'init'loopinv1 s0' a'value \<equiv>
  (
  	 (
  	 	 (
  	 	 	 (inv'
...
definition VC3 where
  "VC3 Init'init'loopinv1 s0' \<equiv>
  (
  	 (Init'init'loopinv1
  	   s0'
  	 )
...
definition VC4 where
  "VC4 Init'init'loopinv1 s0' \<equiv>
  (
  	 (
  	 	 (
  	 	 	 (Init'init'loopinv1
...
definition VC5 where
  "VC5 Init'init'loopinv1 s0' \<equiv>
  (
  	 (
  	 	 (
  	 	 	 (Init'init'loopinv1
...
definition VC6 where
  "VC6 Init'init'loopinv1 Init'init'loopinv2 s0' \<equiv>
  (
  	 (Init'init'loopinv1
  	   s0'
  	 )
...
definition VC7 where
  "VC7 Init'init'loopinv2 s0' \<equiv>
  (
  	 (
  	 	 (Init'init'loopinv2
  	 	   s0'
...
definition VC8 where
  "VC8 inv' env' Init'state2'loopinv1 s0' a'value \<equiv>
  (
  	 (
  	 	 (
  	 	 	 (inv'
...
definition VC9 where
  "VC9 Init'state2'loopinv1 s0' \<equiv>
  (
  	 (
  	 	 (Init'state2'loopinv1
  	 	   s0'
...
definition VC10 where
  "VC10 inv' Init'init'loopinv2 s0' \<equiv>
  (
  	 (
  	 	 (Init'init'loopinv2
  	 	   s0'
...
definition VC11 where
  "VC11 inv' Init'state2'loopinv1 s0' \<equiv>
  (
  	 (
  	 	 (Init'state2'loopinv1
  	 	   s0'
