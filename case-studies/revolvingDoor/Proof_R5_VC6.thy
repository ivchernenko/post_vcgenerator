theory Proof_R5_VC6
  imports  ExtraInv_VC6
begin

abbreviation s where "s s0 user_value pressure_value \<equiv>
(toEnv (setPstate (setVarBool (setVarBool (setVarAny s0 user_value pressure_value) rotation False) brake True) ERROR Controller'suspended))"

theorem proof_5_6: "VC6 inv5 env s0 user_value pressure_value"
  apply(unfold VC6_def inv5_def R5_def)
  apply(rule impI)
