...
\<not> DELAY'TIMEOUT \<le> ltime (reset (setVarAny s0 user_value pressure_value) Controller) Controller"
begin

lemma VC8_R3_ind_step_aux1: " toEnvP s2a \<Longrightarrow>
         substate s2 s2a \<and>
         substate s2a
...
lemma VC8_R3_ind_step: " toEnvP s2a \<Longrightarrow>
           substate s2 s2a \<and>
           substate s2a
           (toEnv (reset (setVarAny s0 user_value pressure_value) Controller)) \<and>
           s2 \<noteq> s2a \<Longrightarrow>
...
"(toEnv (reset (setVarAny s0 user_value pressure_value) Controller))"]
  ...
  by (rule VC8_R3_ind_step_aux1)

 
lemma VC8_R3_ind_proof:   "toEnvP s5 \<and>  substate s2 s5 \<and>
 ...
 s5"
  ...
  using toEnvPs2 apply simp
   ...
   apply(rule impI)
  apply(rule exI[of _ "(toEnv (reset (setVarAny s0 user_value pressure_value) Controller))"])
   apply(rule conjI)
    apply simp
   apply(rule conjI)
    apply simp
...
apply(rule conjI)
  ...
  using substate_asym apply blast 
   apply(simp only: pred3_def) 
  by ((rule VC8_R3_ind_step);fast) 
end


lemma VC8_R3: "  ((((((toEnvP s0 \<and>
             (\<forall>s1 s2.
                 substate s1 s2 \<and>
                 substate s2 s0 \<and>
                 toEnvP s1 \<and>
...
s2"])
 ...
 substate s2 (toEnv (reset (setVarAny s0 user_value pressure_value) ERROR))"])
   apply((rule VC8_R3_ind_proof);blast)
  using substate_refl by fast
  
  

theorem proof_3_8: "VC8 inv3 env s0 user_value pressure_value"
  apply(simp only: VC8_def inv3_def R3_def)
  apply(rule impI)
  apply(rule conjI)
  apply(rule conjI)
   apply(simp)
