...
theorem proof_1_7:
"VC7 inv1 s0 hands_value"
  apply(simp only: VC7_def inv1_def R1_def dryer_def)
...
proof -
  ...
  assume VC: " ((toEnvP s0 \<and>
    ...
    ON \<and>
    \<not> 10 \<le> ltimeEnv (setVarAny s0 hands_value)
             Ctrl"
  show " toEnvP (toEnv (setVarAny s0 hands_value)) \<and>
    (\<forall>s1 s2.
        substate s1 s2 \<and>
        substate s2
         (toEnv (setVarAny s0 hands_value)) \<and>
        toEnvP s1 \<and>
        toEnvP s2 \<and>
        toEnvNum s1 s2 = hands \<and>
        getVarBool s1 hands = OFF \<and>
  ...
  proof(rule allI; rule allI; rule impI)
    ...
    show " \<exists>s4. toEnvP s4 \<and>
            ...
            (\<forall>s3. toEnvP s3 \<and>
                  ...
                  getVarBool s3 hands = ON)"
    proof cases
      assume 1: "s2 =   (toEnv (setVarAny s0 hands_value))"
      have " (toEnvP s2 \<and>
         substate s2 s2 \<and>
         substate s2
          (toEnv (setVarAny s0 hands_value)) \<and>
    ...
    next
       assume 1: "s2 \<noteq>   (toEnv (setVarAny s0 hands_value))"
 ...
 next
   assume VC: "((toEnvP s0 \<and>
    ...
    \<not> 10 \<le> ltimeEnv (setVarAny s0 hands_value)
             Ctrl"
   with extra7
   show "extraInv (toEnv (setVarAny s0 hands_value))"
