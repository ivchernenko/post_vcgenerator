theory Proof_3_7_no_setVarAny
  imports HandDryer_no_setVarAny VCTheoryLemmas_no_setVarAny Extra_no_setVarAny
begin

(*abbreviation s where "s s0 hands_value\<equiv> (toEnv (setVarBool s0 hands hands_value))"*)

(*
theorem proof_3_7: "VC7 inv3 s0 hands_value"
  ...
  subgoal premises vc_prems
    ...
    apply((rule allI)+)
      apply(rule impI)
      subgoal premises req_prems for s1 s2
        apply(rule disjE[of "10 < toEnvNum s2 (s s0 hands_value)" "10 = toEnvNum s2 (s s0 hands_value)"])
        using req_prems le_imp_less_or_eq apply fast
        apply(rule cut_rl[of "\<exists>s4. toEnvP s4 \<and>
         substate s2 s4 \<and>
         substate s4 s0 \<and>
         toEnvNum s2 s4 \<le> 10 \<and>
         (getVarBool s4 dryer = OFF \<or> getVarBool s4 hands = ON) \<and>
         (\<forall>s3. toEnvP s3 \<and> substate s2 s3 \<and> substate s3 s4 \<and> s3 \<noteq> s4 \<longrightarrow> getVarBool s3 dryer = ON \<and> getVarBool s3 hands = OFF)"])
          apply(drule exE)
           prefer 2
           apply assumption
        subgoal for s4
          ...
          by simp
        using invs0(1) req_prems apply -[1]
         apply(drule conjE)
          prefer 2
          apply assumption
        ...
        apply(rule cut_rl[of "\<forall> s5. toEnvP s5 \<and> substate s2 s5 \<and> substate s5 (s s0 hands_value) \<longrightarrow>
pred3 s2 (s s0 hands_value) s5"])
         apply(drule allE[of _ s2])
          prefer 2
          apply assumption
         apply(drule impE)
           prefer 3
           apply assumption
        using req_prems substate_refl apply fast
         apply(simp only: pred3_def)
         apply(drule impE)
           prefer 3
           apply assumption
        using req_prems substate_refl substate_trans substate_asym apply blast
         apply (simp add: dryer_def)
        apply(rule allI)
        subgoal premises toEnvNums2s for s5
          apply(induction rule: state_down_ind)
          using req_prems apply simp
           apply(simp only: pred3_def)
           apply(rule impI)

          apply(rule cut_rl[of "\<exists> s3. toEnvP s3 \<and> substate s2 s3 \<and> substate s3 s0 \<and>
(getVarBool s3 (Suc hands) = OFF \<or> getVarBool s3 hands = ON)"])   
            apply(drule exE)
             prefer 2
             apply assumption
          subgoal for s3
            apply(drule allE[of _ s3])
             prefer 2
             apply assumption
            by (metis state.distinct(11) substate.simps(2) substate.simps(3) substate_asym)
          apply(rule cut_rl[of " 0 < ltimeEnv s0 Ctrl \<and>
     ltimeEnv s0 Ctrl \<le> 10 \<and>
     (\<forall>s1. toEnvP s1 \<and> substate s1 s0 \<and> toEnvNum s1 s0 + 1 = ltimeEnv s0 Ctrl \<longrightarrow>
           getVarBool s1 hands = ON \<and> getVarBool s1 dryer = ON) \<and>
     (\<forall>s1. toEnvP s1 \<and> substate s1 s0 \<and> toEnvNum s1 s0 + 1 < ltimeEnv s0 Ctrl \<longrightarrow>
           getVarBool s1 hands = OFF \<and> getVarBool s1 dryer = ON) \<and>
    (\<forall>s1. toEnvP s1 \<and> substate s1 s0 \<longrightarrow> getPstate s1 Ctrl = waiting \<or> getPstate s1 Ctrl = drying)"])
            apply((drule conjE)+)
                prefer 5
                apply assumption
                prefer 4
          ...
          subgoal premises prems
            using prems(4) apply -
            apply(drule allE[of _ "shiftEnv s0 (ltimeEnv s0 Ctrl)"])
             prefer 2
          apply assumption
             apply(rule exI[of _ "shiftEnv s0 (ltimeEnv s0 Ctrl)"])
            
          
*)

theorem proof_3_7:
"VC7 inv3 s0 hands_value"
  apply(simp only: VC7_def inv3_def  R3_def 
  dryer_def)
  apply(rule impI)
  apply(rule conjI)
   apply(rule conjI)
...
"s = (toEnv (setVarBool s0 hands hands_value))"
  ...
  assume VC: " (((toEnvP s0 \<and>
       (\<forall>s1 s2.
           ...
           10 \<le> toEnvNum s2 s0 \<and> getVarBool s1 hands = ON \<and> getVarBool s1 (Suc hands) = ON \<and> getVarBool s2 hands = OFF \<longrightarrow>
           (\<exists>s4. toEnvP s4 \<and>
                 substate s2 s4 \<and>
                 substate s4 s0 \<and>
                 toEnvNum s2 s4 \<le> 10 \<and>
                 (getVarBool s4 (Suc hands) = OFF \<or> getVarBool s4 hands = ON) \<and>
                 (\<forall>s3. toEnvP s3 \<and> substate s2 s3 \<and> substate s3 s4 \<and> s3 \<noteq> s4 \<longrightarrow>
                       getVarBool s3 (Suc hands) = ON \<and> getVarBool s3 hands = OFF)))) \<and>
      extraInv s0) \<and>
     env (setVarBool s0 hands hands_value) hands_value) \<and>
    getPstate (setVarBool s0 hands hands_value) Ctrl = drying \<and>
    getVarBool (setVarBool s0 hands hands_value) hands \<noteq> ON \<and> \<not> 10 \<le> ltimeEnv (setVarBool s0 hands hands_value) Ctrl "
  show " \<forall>s1 s2.
       substate s1 s2 \<and>
       substate s2 s \<and>
       toEnvP s1 \<and>
       ...
       getVarBool s1 hands = ON \<and> getVarBool s1 (Suc hands) = ON \<and> getVarBool s2 hands = OFF \<longrightarrow>
       (\<exists>s4. toEnvP s4 \<and>
             substate s2 s4 \<and>
             substate s4 s \<and>
             toEnvNum s2 s4 \<le> 10 \<and>
             (getVarBool s4 (Suc hands) = OFF \<or> getVarBool s4 hands = ON) \<and>
             (\<forall>s3. toEnvP s3 \<and> substate s2 s3 \<and> substate s3 s4 \<and> s3 \<noteq> s4 \<longrightarrow>
                   getVarBool s3 (Suc hands) = ON \<and> getVarBool s3 hands = OFF))"
  proof(rule allI; rule allI; rule impI)
    ...
    assume req_prems: " substate s1 s2 \<and>
       ...
       toEnvP s2 \<and>
       toEnvNum s1 s2 = 1 \<and>
       10 \<le> toEnvNum s2 s \<and> getVarBool s1 hands = ON \<and> getVarBool s1 (Suc hands) = ON \<and> getVarBool s2 hands = OFF "
...
substate s5 s \<longrightarrow>pred3 s2 s s5" 
      ...
      (getPstate s0 Ctrl = drying \<longrightarrow>
       ...
       (\<forall>s1. toEnvP s1 \<and> substate s1 s0 \<and> toEnvNum s1 s0 + 1 < ltimeEnv s0 Ctrl \<longrightarrow>
             getVarBool s1 hands = OFF \<and> getVarBool s1 dryer = ON)) \<and>
      (\<forall>s1. toEnvP s1 \<and> substate s1 s0 \<longrightarrow> getPstate s1 Ctrl = waiting \<or> getPstate s1 Ctrl = drying)) \<and>
     env (setVarBool s0 hands hands_value) hands_value) \<and>
    getPstate (setVarBool s0 hands hands_value) Ctrl = drying \<and>
    getVarBool (setVarBool s0 hands hands_value) hands \<noteq> ON \<and> \<not> 10 \<le> ltimeEnv (setVarBool s0 hands hands_value) Ctrl "
              then obtain 7:
...
next
  assume "(((toEnvP s0 \<and>
    ...
    getPstate (setVarBool s0 hands hands_value) Ctrl = drying \<and>
    getVarBool (setVarBool s0 hands hands_value) hands \<noteq> ON \<and> \<not> 10 \<le> ltimeEnv (setVarBool s0 hands hands_value) Ctrl "
  with extra7 show " extraInv (toEnv (setVarBool s0 hands hands_value))"
    by (auto simp add: VC7_def)
qed
