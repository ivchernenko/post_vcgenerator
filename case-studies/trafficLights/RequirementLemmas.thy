...
(\<forall> s2. toEnvP s2 \<and> substate s1 s2 \<and> substate s2 s4 \<longrightarrow> vc3 s2)))"
  ...
  subgoal for s4
    ...
    subgoal for s1
      apply(cases "s1 = s") 
       apply (metis less_eq_nat.simps(1) substate_asym toEnvNum_id)
      ...
      by metis
     apply simp
    using substate_or_substate_pair by (meson substate_asym substate_refl)
  done
...
(\<forall> s3. toEnvP s3 \<and> substate s2 s3 \<and> substate s3 s5 \<longrightarrow> vc3 s3)))"
  ...
  subgoal for s5
    ...
    subgoal for s1 s2
      apply(cases "s2 = s")
       apply (metis less_eq_nat.simps(1) substate_asym toEnvNum_eq_imp_eq2 toEnvNum_id)
      apply(erule allE[of _ s0])
      apply(rotate_tac -1)
      apply(erule impE)
      using substate_refl  apply simp
