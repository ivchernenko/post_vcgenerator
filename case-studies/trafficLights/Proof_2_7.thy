...
theorem proof_2_7: "VC7 inv2 s0 requestButtonValue"
...
proof
  ...
  assume "  ((toEnvP s0 \<and>
    ...
    \<not> (getVarBool (setVarAny s0 requestButtonValue) redAfterMinimalRed \<or> getVarBool (setVarAny s0 requestButtonValue) Ctrl)"
...
and vc: "env (setVarAny s0 requestButtonValue) requestButtonValue \<and>
 ...
 ltimeEnv g Ctrl = T2" ..
    ...
    qed
  next
    assume 1: "s3 \<noteq> (toEnv (setVarAny s0 requestButtonValue))"
    show ?thesis
    proof
      assume req_prems: " substate s1 s2 \<and>
    substate s2 s3 \<and>
