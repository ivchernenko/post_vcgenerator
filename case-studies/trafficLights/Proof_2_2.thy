...
theorem proof_2_2: "VC2 inv2 s0 requestButtonValue"
...
and vc: " env (setVarAny s0 requestButtonValue) requestButtonValue \<and>
  ...
  show "(toEnvP
      (toEnv
        (setPstate (setVarBool (setVarAny s0 requestButtonValue) redAfterMinimalRed PRESSED) Ctrl
    ...
    toEnvP
     (toEnv
       (setPstate (setVarBool (setVarAny s0 requestButtonValue) redAfterMinimalRed PRESSED) Ctrl redAfterMinimalRed)) \<and>
    (\<forall>s1. toEnvP s1 \<and>
          substate s1
           (toEnv
             (setPstate (setVarBool (setVarAny s0 requestButtonValue) redAfterMinimalRed PRESSED) Ctrl
 ...
 toEnvNum s2 s3 < T2 \<and> getVarBool s1 minimalRed \<noteq> PRESSED \<and> getVarBool s2 minimalRed = PRESSED"
  ...
  next
    ...
    next
      assume 1: " s3 \<noteq>
    toEnv
     (setPstate (setVarBool (setVarAny s0 requestButtonValue) redAfterMinimalRed PRESSED) Ctrl redAfterMinimalRed) "
      show ?thesis
      proof
        print_state
        assume 7: "substate s1 s2 \<and>
