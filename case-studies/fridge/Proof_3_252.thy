 ...
 freezerTempGreaterMax_value fridgeDoor_value) "]
      ...
      subgoal for s4
       ...
       apply(rule conjI)
        ...
        by (smt (z3) Suc_numeral add_One_commute numeral_1_eq_Suc_0 numeral_2_eq_2 numeral_3_eq_3 semiring_norm(2) semiring_norm(3) semiring_norm(5) semiring_norm(8) substate_antisym substate_trans)
           done
    done
  done
  done
