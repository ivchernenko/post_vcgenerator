         ...
         (setVarBool
           (setPstate
             (setVarBool
               ...
               lighting' CLOSED')
             Init'begin' FridgeDoorController'closed')
           FridgeCompressorController'checkTemp' OPEN')
         FreezerCompressorController'checkTemp' OPEN'))"

theorem proof_3_327: "VC327 inv3 env s0 fridgeTempGreaterMin_value fridgeTempGreaterMax_value freezerTempGreaterMin_value
 freezerTempGreaterMax_value fridgeDoor_value"
  apply(unfold VC327_def inv3_def R3_def)
 apply(rule impI)
  apply(rule conjI)
   apply(rule conjI)
    apply simp
