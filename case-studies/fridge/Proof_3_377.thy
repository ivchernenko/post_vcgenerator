...
theorem proof_3_252: "VC377 inv3 env s0 fridgeTempGreaterMin_value fridgeTempGreaterMax_value freezerTempGreaterMin_value
 freezerTempGreaterMax_value fridgeDoor_value"
  apply(unfold VC377_def inv3_def R3_def)
 apply(rule impI)
  apply(rule conjI)
   apply(rule conjI)
    apply simp
     ...
     apply assumption
      ...
      subgoal premises prems for s5
        ...
        using prems apply simp
         ...
         apply(rule impE[OF open_R3[of s1 s0 s0]])
          apply(insert vc_prems(1)[simplified])
          apply(rule conjI)
           apply(simp split: if_splits)
        apply(rule conjI)
           apply(simp split: if_splits)
        apply(rule conjI)
        using invs0(2) apply blast
       apply(rule conjI)
           apply(simp split: if_splits)
       apply(rule conjI)
           apply(simp split: if_splits)
