...
(\<forall> s1 s2. substate s1 s2 \<and> substate s2 s \<and> toEnvP s1 \<and> toEnvP s2 \<and> toEnvNum s1 s2 = 1 \<and>
getVarBool s1 maintainingMode' \<and> getVarInt s1 temperature' \<ge> getVarInt s1 selectedTemp' \<longrightarrow> getVarBool s1 heater' = False)"

definition R3 where "R3 s \<equiv> toEnvP s \<and>
(\<forall> s1 s2. substate s1 s2 \<and> substate s2 s \<and> toEnvP s1 \<and> toEnvP s2 \<and> toEnvNum s1 s2 = 1 \<and>
getVarBool s1 maintainingMode' \<and> getVarInt s2 temperature' < getVarInt s2 selectedTemp' - 5 \<longrightarrow>
